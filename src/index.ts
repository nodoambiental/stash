...
abstract class BaseStash implements StashImplementation {
	...
	set<T>(id: string, value: T): void {
		...
		const newPointer = () => this.entries[cleanId].records[`val${this.own.step.current}`].value
	...
	transform<T>(id: string, transformation: (data: T) => T): void {
		...
		this.entries[cleanId].records[`val${this.own.step.current}`] = clone({
			...
			value: transform(this.entries[cleanId].value() as T, transformation),
			timestamp: new Date().toDateString(),
			step: parseInt(this.own.step.current.toString(), 10)
		})
	...
	protected event(_eventType: keyof CustomAvailableEvents, _id: string, _step: number): void {
		return
	}
	 ...
	 */
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	protected syncAccessor(_id: string): void {
		return
	}
}
...
export class Stash extends BaseStash implements StashImplementation {
	...
	private sync(id: string): void {
		...
		if (this.own.persistence === "local") {
			localforage.setItem(this.own.isCustom ? this.own.stashId : "localStash", this)
		}
	}
	 ...
	 * TODO Docs
	 */
	protected event(eventType: keyof AvailableEvents, id: string, step: number): void {
 ...
 * TODO
 */
export class CustomStash extends BaseStash implements CustomStashImplementation {
	/**
	 * TODO Docs
	 */
	constructor(public readonly own: StashOwnData, enableDeletion = false) {
		super(own)
	 ...
	 * TODO Docs
	 */
	protected event(eventType: keyof CustomAvailableEvents, id: string, step: number): void {
		dispatchEvent(this.events[eventType](id, step))
	}
}
 ...
 * TODO Docs
 */
export const initializeStashes = async (name = "localStash"): Promise<void> => {
